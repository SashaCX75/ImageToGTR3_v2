...
namespace ImageToZeppOS
{
    class ColorMap
    {
        ...
        private void ARGB_BGRA(bool fix_color)
        {
            for (int i = 0; i < Colors.Count; i++)
            {
                ...
                G = (byte)(G * scale);
                B = (byte)(B * scale);
                if (!fix_color) Colors[i] = Color.FromArgb(B, G, R, A);
                else Colors[i] = Color.FromArgb(R, G, B, A);
            }
        }

        public void ColorsFix(bool argb_brga, int colorMapCount, byte colorMapEntrySize, bool fix_color)
        {
